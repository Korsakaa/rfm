...
pub struct ChmodState {
    ...
    pub is_dir:       bool,
    pub needs_sudo:   bool,      // текущий пользователь не владелец
    pub change_owner: bool,      // сменить владельца на текущего
    pub recursive:    bool,      // применить рекурсивно
    pub owner_name:   String,    // имя текущего владельца файла
    pub current_user: String,    // имя текущего пользователя
}
...
/// Получить UID текущего процесса
fn get_uid() -> u32 {
...
/// Команды для выполнения через sudo
pub struct SudoOps {
    pub commands: Vec<(String, Vec<String>)>, // (программа, аргументы)
}
...
impl App {
    ...
    /// Вызывается каждый фрейм. Обрабатывает файлы пока есть ~30ms времени,
    /// потом отдаёт управление для перерисовки.
    pub fn tick(&mut self) {
        ...
        while state.done < state.files.len() {
            ...
            // Уступаем управление каждые ~30ms чтобы перерисовать прогресс-бар
            if start.elapsed().as_millis() >= 30 { break; }
        }

        if state.done >= state.files.len() {
            let op   = match state.op { ProgressOp::Copy => "Скопировано", ProgressOp::Move => "Перемещено" };
            ...
            self.message = Some(if err == 0 {
                ...
                format!("{op}: {done}/{total}  (ошибок: {err})")
            });
            ...
            || key == KeyEvent::new(KeyCode::F(8), KeyModifiers::NONE)
        {
            ...
            if let Some(e) = entry {
                if !e.is_dir && e.name != ".." {
                    ...
                    self.mode = Mode::Input(InputState {
                        prompt: format!("Открыть .{ext} через: "),
                        value:  current,
            ...
            self.left.reload();
            self.right.reload();
            self.message = Some("Обновлено".into());
            ...
            KeyCode::Char('y') | KeyCode::Enter => {
                match action {
                    ConfirmAction::Delete(paths) => {
                        ...
                        self.left.reload();
                        self.right.reload();
                        self.message = Some(format!("Удалено: {n} объект(ов)"));
                    }
                }
            }
                    ...
                    InputAction::Mkdir => {
                        if !state.value.is_empty() {
                            ...
                            match std::fs::create_dir_all(&path) {
                                Ok(_)  => self.message = Some(format!("Создана: {}", state.value)),
                                Err(e) => self.message = Some(format!("Ошибка: {e}")),
                            }
                            self.active_panel_mut().reload();
                        }
    ...
    fn open_file(&mut self, name: &str) {
        ...
        match self.openers.get(&ext).cloned() {
            Some(prog) => {
                // Перенаправляем stdout/stderr в /dev/null чтобы вывод
                // внешней программы не портил наш TUI
                if let Err(e) = std::process::Command::new(&prog)
                    ...
                    .spawn()
                {
                    self.message = Some(format!("Ошибка запуска {prog}: {e}"));
                }
            }
            None => {
                self.message = Some(
                    format!("Нет программы для .{ext}  (добавьте в ~/.config/fm/openers.toml)")
                );
            }
        }
    }
    ...
    fn handle_search(&mut self, key: KeyEvent, mut state: SearchState, height: usize) {
        match key.code {
            KeyCode::Esc => {}  // закрыть — mode остаётся Normal

            // Tab / BackTab — цикл по фокусу
            KeyCode::Tab => {
                    ...
                    SearchFocus::Input | SearchFocus::BtnNext => {
                        if !state.query.is_empty() {
                            ...
                            match self.find_match_next(&state.query, from) {
                                ...
                                None => self.message = Some(format!("«{}» не найдено", state.query)),
                            }
                        }
                        self.mode = Mode::Search(state);
                    }
            ...
            // Символы всегда идут в поле ввода
            KeyCode::Char(c) => {
                state.focus = SearchFocus::Input;
                state.query.push(c);
                // Прыгаем на первое совпадение с начала
    ...
    /// Поиск вперёд начиная с элемента после `from` (с обёрткой)
    ...
    /// Поиск назад начиная с элемента перед `from` (с обёрткой)
            ...
            KeyCode::Enter => {
                if let Some(entry) = state.entries.get(state.cursor) {
                    ...
                    if path.is_dir() {
                        ...
                        self.active_panel_mut().selected.clear();
                        self.active_panel_mut().reload();
                        // mode → Normal, меню закрыто
                    } else {
                        self.message = Some(format!("Путь недоступен: {}", entry.mountpoint));
                        self.mode = Mode::UsbMenu(state);
                    }
                }
            }
        ...
        match key.code {
            KeyCode::Enter => {
                ...
                if state.needs_sudo || state.change_owner {
                    // Нужен sudo — собираем команды и откладываем выполнение
                    ...
                    if rec {
                        match std::process::Command::new("chmod")
                            ...
                            .status()
                        {
                            Ok(_)  => self.message = Some(format!("Права {:o} применены рекурсивно", mode)),
                            Err(e) => self.message = Some(format!("Ошибка: {e}")),
                        }
                    } else {
                        ...
                        {
                            Ok(_)  => self.message = Some(format!("Права: {:o} → {}", mode, state.name)),
                            Err(e) => self.message = Some(format!("Ошибка: {e}")),
                        }
                    }
                    self.active_panel_mut().reload();
            ...
            KeyCode::Up | KeyCode::Char('k') => {
                state.cursor = match state.cursor {
                    ...
                    9  => 6,  // с change_owner → нижняя строка сетки
                    10 => 9,  // с recursive → change_owner
                    _  => state.cursor,
                };
                self.mode = Mode::Chmod(state);
            }
            KeyCode::Down | KeyCode::Char('j') => {
                state.cursor = match state.cursor {
                    0..=5 => state.cursor + 3,  // вверх по сетке
                    6..=8 => 9,                  // нижняя строка сетки → change_owner
                    9 if state.is_dir => 10,     // change_owner → recursive
                    _  => state.cursor,
                };
                self.mode = Mode::Chmod(state);
            }
    ...
    fn do_copy(&mut self) {
        ...
        if files.is_empty() {
            self.message = Some("Нечего копировать".into());
            return;
        }
    ...
    fn do_move(&mut self) {
        let files = self.active_panel().selected_paths();
        if files.is_empty() {
            self.message = Some("Нечего перемещать".into());
            return;
        }
        let dst = self.inactive_panel().path.clone();
...
/// Возвращает смонтированные съёмные устройства через lsblk
fn list_usb_mounts() -> Vec<UsbEntry> {
    // Пробуем MOUNTPOINTS (lsblk ≥ 2.37), при ошибке — MOUNTPOINT
    ...
    // Первый проход: собираем имена дисков с TRAN=usb
    ...
    // Второй проход: собираем смонтированные разделы
    ...
    for line in text.lines() {
        ...
        // Устройство USB, если само имеет TRAN=usb, HOTPLUG=1,
        // или его имя начинается с имени USB-диска (sdb → sdb1, sdb2…)
...
/// Разбирает одну строку lsblk -P вида: KEY="VALUE" KEY="VALUE" …
...
/// Записывает/обновляет запись в ~/.config/fm/openers.toml
fn save_opener(ext: &str, prog: &str) {
    use crate::config::config_dir;
    let path = config_dir().join("openers.toml");

    // Читаем текущий файл как toml::Table или создаём пустую
    let mut root: toml::Table = std::fs::read_to_string(&path)
        .ok()
        .and_then(|s| toml::from_str(&s).ok())
        .unwrap_or_default();

    // Получаем/создаём секцию [openers]
    let openers = root
