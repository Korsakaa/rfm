...
impl App {
    ...
    pub fn tick(&mut self) {
        ...
        if state.done >= state.files.len() {
            let op    = match state.op { ProgressOp::Copy => "Copied", ProgressOp::Move => "Moved" };
            ...
            self.message = Some(if err == 0 {
                ...
                format!("{op}: {done}/{total}  (errors: {err})")
            });
            ...
            KeyCode::Char('y') | KeyCode::Enter => {
                match action {
                    ConfirmAction::Delete(paths) => {
                        ...
                        self.left.reload();
                        self.right.reload();
                        self.message = Some(format!("Deleted: {n} item(s)"));
                    }
                }
            }
                    ...
                    InputAction::Mkdir => {
                        if !state.value.is_empty() {
                            ...
                            match std::fs::create_dir_all(&path) {
                                Ok(_)  => self.message = Some(format!("Created: {}", state.value)),
                                Err(e) => self.message = Some(format!("Error: {e}")),
                            }
                            self.active_panel_mut().reload();
                        }
    ...
    fn open_file(&mut self, name: &str) {
        ...
        match self.openers.get(&ext).cloned() {
            Some(prog) => {
                // Redirect stdout/stderr to /dev/null so external program
                ...
                if let Err(e) = std::process::Command::new(&prog)
                    ...
                    .spawn()
                {
                    self.message = Some(format!("Failed to launch {prog}: {e}"));
                }
            }
            None => {
                self.message = Some(
                    format!("No opener for .{ext}  (add to ~/.config/rfm/openers.toml)")
                );
            }
        }
    }
                    ...
                    SearchFocus::Input | SearchFocus::BtnNext => {
                        if !state.query.is_empty() {
                            ...
                            match self.find_match_next(&state.query, from) {
                                ...
                                None => self.message = Some(format!("«{}» not found", state.query)),
                            }
                        }
                        self.mode = Mode::Search(state);
                    }
            ...
            KeyCode::Enter => {
                if let Some(entry) = state.entries.get(state.cursor) {
                    ...
                    if path.is_dir() {
                        ...
                        self.active_panel_mut().selected.clear();
                        self.active_panel_mut().reload();
                        // mode → Normal, menu closed
                    } else {
                        self.message = Some(format!("Path unavailable: {}", entry.mountpoint));
                        self.mode = Mode::UsbMenu(state);
                    }
                }
            }
        ...
        match key.code {
            KeyCode::Enter => {
                ...
                if state.needs_sudo || state.change_owner {
                    // Sudo required — queue commands for deferred execution
                    ...
                    if rec {
                        match std::process::Command::new("chmod")
                            ...
                            .status()
                        {
                            Ok(_)  => self.message = Some(format!("Permissions {:o} applied recursively", mode)),
                            Err(e) => self.message = Some(format!("Error: {e}")),
                        }
                    } else {
                        ...
                        {
                            Ok(_)  => self.message = Some(format!("Permissions: {:o} → {}", mode, state.name)),
                            Err(e) => self.message = Some(format!("Error: {e}")),
                        }
                    }
                    self.active_panel_mut().reload();
            ...
            KeyCode::Up | KeyCode::Char('k') => {
                state.cursor = match state.cursor {
                    ...
                    9  => 6,  // change_owner → bottom perm row
                    10 => 9,  // recursive → change_owner
                    _  => state.cursor,
                };
                self.mode = Mode::Chmod(state);
            }
            KeyCode::Down | KeyCode::Char('j') => {
                state.cursor = match state.cursor {
                    0..=5 => state.cursor + 3,
                    6..=8 => 9,                 // bottom perm row → change_owner
    ...
    fn do_move(&mut self) {
        let files = self.active_panel().selected_paths();
        if files.is_empty() {
            self.message = Some("Nothing to move".into());
            return;
        }
        let dst = self.inactive_panel().path.clone();
...
/// Returns mounted removable devices via lsblk
fn list_usb_mounts() -> Vec<UsbEntry> {
    // Try MOUNTPOINTS (lsblk ≥ 2.37), fall back to MOUNTPOINT
    ...
    // First pass: collect disk names with TRAN=usb
    ...
    // Second pass: collect mounted partitions
    ...
    for line in text.lines() {
        ...
        // Device is USB if HOTPLUG=1 or its name starts with a USB disk name
...
/// Parses one lsblk -P line of the form: KEY="VALUE" KEY="VALUE" …
...
/// Writes / updates an entry in ~/.config/rfm/openers.toml
