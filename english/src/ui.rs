...
pub fn render(f: &mut Frame, app: &App) {
    ...
    let rows = Layout::default()
        ...
        .constraints([
            ...
            Constraint::Length(2), // hotkeys bar
        ])
        .split(area);
    ...
    // List height = panel height minus 2 borders
...
fn render_status(f: &mut Frame, app: &App, area: Rect) {
    let text = if let Some(msg) = &app.message {
        ...
        let total = p.entries.len().saturating_sub(1); // without ".."
        let sel   = p.selected.len();
        match p.current_entry() {
            Some(e) if e.is_dir && e.name != ".." =>
                format!("  [DIR] {}  |  items: {}  selected: {}", e.name, total, sel),
            Some(e) if e.name != ".." =>
                format!("  {}  {}  |  items: {}  selected: {}", e.name, fmt_size(e.size), total, sel),
            _ =>
                format!("  items: {}  selected: {}", total, sel),
        }
    };
...
fn render_hotkeys(f: &mut Frame, area: Rect) {
    let line1 = " Tab/[:Left panel   ]:Right panel   ↑↓:Navigate   Alt+↑↓:Jump   Space:Select   Alt+u:USB drives";
    let line2 = " F2:New file   F3:Search   F5:Copy   F6:Move   F7:Folder   F8:Delete   Alt+f:Perms   Alt+↵:Opener   \\ :Root   r:Refresh   q:Quit";
    let text = format!("{}\n{}", line1, line2);
    f.render_widget(
...
fn render_confirm(f: &mut Frame, action: &ConfirmAction, area: Rect) {
    ...
    let msg = match action {
        ConfirmAction::Delete(paths) =>
            format!("  Delete {}  item(s)?    [y] Yes    [n/Esc] No", paths.len()),
    };
...
fn render_progress(f: &mut Frame, state: &ProgressState, area: Rect) {
    ...
    let op_name = match state.op {
        ProgressOp::Copy => "Copying",
        ProgressOp::Move => "Moving",
    };
    ...
    let rows = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(1), // "file X of Y"
    ...
    f.render_widget(
        Paragraph::new(format!("  File {} of {}", done, total))
            .style(Style::default().fg(Color::White)),
        rows[0],
    );
...
fn render_chmod(f: &mut Frame, state: &ChmodState, area: Rect) {
    ...
    let rows = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(1), // 0: column header
            Constraint::Length(1), // 1: separator
            Constraint::Length(1), // 2: owner row
            Constraint::Length(1), // 3: group row
            Constraint::Length(1), // 4: others row
    ...
    let label_names = ["Owner", "Group", "Others"];
    ...
    // Owner info / sudo warning
    let owner_line = if state.needs_sudo {
        format!("  ! Owner: {}  (sudo required)", state.owner_name)
    } else {
        format!("  Owner: {}", state.owner_name)
    };
    ...
    // Checkbox: transfer ownership to current user
    {
        ...
        let spans = vec![
            Span::styled("  Change owner to ", Style::default().fg(Color::White)),
    ...
    // Checkbox: apply recursively (directories only)
    {
        ...
        if state.is_dir {
            let spans = vec![
                Span::styled("  Apply recursively  ", Style::default().fg(Color::White)),
                Span::styled(cb, cb_style),
            ];
    ...
    // Hint
    f.render_widget(
        Paragraph::new("  ←↑↓→/hjkl: navigate   Space: toggle   Enter: apply   Esc: cancel")
            .style(Style::default().fg(Color::DarkGray)),
        rows[10],
    );
}
...
fn render_search(f: &mut Frame, state: &SearchState, panel_area: Rect) {
    ...
    let buttons = Line::from(vec![
        Span::raw(" "),
        btn("◄ Prev",  state.focus == SearchFocus::BtnPrev),
        Span::raw("  "),
        btn("Next ►",  state.focus == SearchFocus::BtnNext),
        Span::raw("  "),
        btn("✕ Close", state.focus == SearchFocus::BtnClose),
        Span::raw(if not_found { "  not found" } else { "" }),
    ]);
    ...
    if rows[3].height > 0 {
        f.render_widget(
            Paragraph::new("  Tab: buttons   Enter: search   Esc: close")
                .style(Style::default().fg(Color::DarkGray)),
            rows[3],
        );
    }
...
fn render_usb_menu(f: &mut Frame, state: &UsbMenuState, area: Rect) {
    ...
    if state.entries.is_empty() {
        f.render_widget(
            Paragraph::new("  No USB drives found or none are mounted")
                .style(Style::default().fg(Color::DarkGray)),
            inner,
        );
        return;
    ...
    f.render_widget(
        Paragraph::new("  ↑↓: select   Enter: navigate   Alt+u: refresh   Esc: close")
            .style(Style::default().fg(Color::DarkGray)),
        rows[3],
    );
}
